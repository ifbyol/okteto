...
func (d *Driver) Deploy(ctx context.Context) error {
	var err error
	d.cache, err = initDivertIstioCache(ctx, d.Manifest, d.IstioClient)
	if err != nil {
		return err
	}
...
func (d *Driver) ApplyNamespaceToProxy() string {
...
func initDivertIstioCache(ctx context.Context, m *model.Manifest, c istioclientset.Interface) (*divertIstioCache, error) {
	cache := &divertIstioCache{
		divertVirtualServices:    map[string]*istioV1beta1.VirtualService{},
		developerVirtualServices: map[string]*istioV1beta1.VirtualService{},
	}
	vsList, err := virtualservices.List(ctx, m.Deploy.Divert.Namespace, c)
	if err != nil {
		return nil, err
	}
	for i := range vsList {
		cache.divertVirtualServices[vsList[i].Name] = vsList[i]
	}

	vsList, err = virtualservices.List(ctx, m.Namespace, c)
	if err != nil {
		return nil, err
	}
	for i := range vsList {
		cache.developerVirtualServices[vsList[i].Name] = vsList[i]
	}
	return cache, nil
}
